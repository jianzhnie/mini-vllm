...
Examples:
  ...
  python run_tests.py --slow             # Include slow tests
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Decrease verbosity (quiet mode)"
    )
    parser.add_argument(
        "-k",
        "--keyword",
    ...
    parser.add_argument(
        "--failed", action="store_true", help="Run only previously failed tests"
    )
    parser.add_argument(
    ...
    # Add coverage
    if args.coverage:
        cmd.extend(["--cov=minivllm", "--cov-report=html", "--cov-report=term-missing"])
