...
SPARSE_MODE = int(
    os.getenv('NPU_FA2_SPARSE_MODE',
              default=DOWN_RIGHT_ALIGNED_CAUSAL_MASK_MODE))
...
def get_attn_mask_npu(device):
    ...
    if device not in ATTN_MASK_NPU_CACHE:
        ATTN_MASK_NPU_CACHE[device] = torch.triu(torch.ones([2048, 2048],
                                                            device=device),
                                                 diagonal=1).bool()
...
def is_npu_fa2_top_left_aligned_causal_mask():
    return SPARSE_MODE == TOP_LEFT_ALIGNED_CAUSAL_MASK_MODE if is_torch_npu_available(
    ) else False
