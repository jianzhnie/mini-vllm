...
from typing import Callable, Optional, Tensor
...
def multinomial_sample_one(probs: Tensor, q: Tensor) -> Tensor:
    ...
    Args:
        ...
        q: Random noise tensor for the Gumbel-max trick
...
def sample(
    ...
    *,
    ...
    Args:
        logits (Tensor): logits from which to sample
        temperature (float): value to scale the predicted logits by, default 1.0.
        top_k (Optional[int]): If specified, we prune the sampling to only token ids within the top_k probabilities
        q (Optional[Tensor]): randomly sampled tensor for softmax sampling trick. If None,
...
def generate_next_token(
    ...
    *,
    ...
    Args:
        ...
        input_pos (Tensor): tensor with the positional encodings associated with the given prompt,
            with shape [bsz x seq_length].
        x (Tensor): tensor with the token IDs associated with the given prompt,
            with shape [bsz x seq_length].
        q (Optional[Tensor]): randomly sampled tensor for softmax sampling trick.
        mask (Optional[Tensor]): attention mask, default None.
        temperature (float): value to scale the predicted logits by, default 1.0.
        top_k (Optional[int]): Top-k value to use for sampling, default None.

    Returns:
        tuple[Tensor, Tensor]: tuple of two tensors:
            - tokens (Tensor): tensor with the generated tokens,
                with shape [bsz x 1].
            - logits (Tensor): tensor with the logits associated with the generated tokens,
...
def update_stop_tokens_tracker(tokens: Tensor, stop_tokens: Tensor,
                               stop_token_reached: Tensor) -> Tensor:
    """
    Updates which sequences have reached a stop token.

    Args:
        tokens: Generated tokens of shape [bsz, 1]
        stop_tokens: Stop token IDs of shape [num_stop_tokens]
        stop_token_reached: Boolean tensor tracking which sequences have reached a stop token, shape [bsz]

    Returns:
        Updated stop_token_reached tensor
    """
    # tokens: [bsz, 1]
...
def get_causal_mask_from_padding_mask(padding_mask: Tensor,
    ...
    Args:
        padding_mask (Tensor): Boolean tensor where False indicates the corresponding token in the sequence
    ...
    Returns:
        Tensor: Boolean causal mask with shape
...
def get_position_ids_from_padding_mask(padding_mask: Tensor, ):
    """
    ...
    Returns:
        Tensor: position ids which are appropriately shifted according to any padding values.
...
def generate(
    ...
    *,
    ...
    Args:
        model (TransformerDecoder): model used for generation
        prompt (Tensor): tensor with the token IDs associated with the given prompt,
