...
class SamplingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.top_k < -1:
            raise ValueError(f"top_k must be -1 (disabled) or >= 0, got {self.top_k}")
        if not 0.0 <= self.top_p <= 1.0:
            raise ValueError(f"top_p must be between 0.0 and 1.0, got {self.top_p}")
        if not 0.0 <= self.min_p <= 1.0:
            raise ValueError(f"min_p must be between 0.0 and 1.0, got {self.min_p}")
        if self.typical_p <= 0:
            raise ValueError(f"typical_p must be positive, got {self.typical_p}")
